...
def find_and_group_panels(doc, config):
    ...
    and groups them into physical panels (back_capable, front_only) based on
    containment within sheet borders and the presence of _ABF_PART_BORDER.
    ...
    for i, sb_bbox in enumerate(sheet_border_bboxes):
        ...
        for pb in part_borders:
            ...
            if pb_center and is_point_inside_bbox(pb_center, sb_bbox, tolerance=1.1): # Increased tolerance for sheet containment
        ...
        if contains_part_border:
            ...
            # Corrected assignment - should use the current sb_bbox
    ...
    # 1. Process panels with _ABF_PART_BORDER (these are back-capable, located in the back sheet area)
    for pb in part_borders:
        pb_vertices = list(pb.vertices())
        if not pb_vertices: continue # Skip empty borders
        pb_bbox = get_bbox(pb_vertices)
        pb_center = get_bbox_center(pb_vertices)

        # Confirm the part border is within the back sheet border (should be based on logic above, but good check)
        if pb_center and is_point_inside_bbox(pb_center, back_sheet_bbox, tolerance=1.1): # Increased tolerance
            # Find the corresponding cutting line border in the front sheet area by dimension match ONLY
            pb_dims = get_bbox_dimensions_sorted(pb_vertices)
            matched_cl_border = None
            matched_cl_index = -1

            # Iterate through cutting lines in the front sheet area that haven't been used yet
            for idx, cl_border in enumerate(cutting_lines_borders):
                 if idx not in used_cutting_lines_indices:
                      ...
                      # Check if the cutting line is within the front sheet area
                      if cl_center and is_point_inside_bbox(cl_center, front_sheet_bbox, tolerance=1.1):
                           ...
                           # Check for dimension match with the part border
                           if math.isclose(pb_dims[0], cl_dims[0], abs_tol=0.1) and \
                              math.isclose(pb_dims[1], cl_dims[1], abs_tol=0.1):
                               ...
                               break # Found a match, move to the next part border

            if matched_cl_border:
                 grouped_panels.append({
                     'borders': [pb, matched_cl_border],
                     'type': 'back_capable',
                     'primary_border': pb, # Part border is primary for back_capable
                     'secondary_border': matched_cl_border, # Cutting line is secondary
                     'primary_bbox': pb_bbox, # Store panel bboxes
                     'secondary_bbox': get_bbox(list(matched_cl_border.vertices())),
                     'sheet_border_front_bbox': front_sheet_bbox, # Store sheet bboxes with panel
                     'sheet_border_back_bbox': back_sheet_bbox
                 })
                 used_cutting_lines_indices.add(matched_cl_index)
                 print(f"DEBUG: پنل back_capable گروه بندی شد (مرز ABF_PART_BORDER در مختصات {pb_center}) با مرز ABF_CUTTING_LINES در مختصات {get_bbox_center(list(matched_cl_border.vertices()))}) بر اساس تطابق ابعاد.")
            else:
                 # This case might indicate an issue in the DXF if a part border doesn't have a matching cutting line in the front sheet
                 print(f"WARNING: مرز ABF_PART_BORDER در مختصات {pb_center} در مرز ورق پشت یافت شد اما مرز ABF_CUTTING_LINES هم اندازه در مرز ورق جلو یافت نشد. این پنل ممکن است به درستی پردازش نشود.")
                 # Add it as back_capable_standalone if it's a valid scenario for your workflow
                 grouped_panels.append({
                     'borders': [pb],
                     'type': 'back_capable_standalone',
                     'primary_border': pb,
                     'secondary_border': None, # No matching cutting line
                     'primary_bbox': pb_bbox, # Store panel bbox
                     'secondary_bbox': None,
                     'sheet_border_front_bbox': front_sheet_bbox, # Still provide both sheet borders for reference
                     'sheet_border_back_bbox': back_sheet_bbox
                 })
                 print(f"DEBUG: پنل back_capable_standalone گروه بندی شد (مرز ABF_PART_BORDER در مختصات {pb_center}).")


    # 2. Process remaining _ABF_CUTTING_LINES (these are front-only, located in the front sheet area)
    # These are the cutting lines that were NOT matched with a part border.
    for idx, cl_border in enumerate(cutting_lines_borders):
        if idx not in used_cutting_lines_indices:
            cl_vertices = list(cl_border.vertices())
            if not cl_vertices: continue
            ...
            # Confirm the cutting line is within the front sheet border
            if cl_center and is_point_inside_bbox(cl_center, front_sheet_bbox, tolerance=1.1): # Increased tolerance
                 grouped_panels.append({
                     'borders': [cl_border],
                     'type': 'front_only',
                     'primary_border': cl_border, # Cutting line is primary for front_only
                     'secondary_border': None,
                     'primary_bbox': cl_bbox, # Store panel bbox
                     'secondary_bbox': None,
                     'sheet_border_front_bbox': front_sheet_bbox, # Store sheet bboxes with panel
                     'sheet_border_back_bbox': back_sheet_bbox # Still provide both sheet borders for reference
                 })
                 print(f"DEBUG: مرز ABF_CUTTING_LINES (مرکز ({cl_center[0]:.1f}, {cl_center[1]:.1f}))) به عنوان پنل فیزیکی تنها front_only اضافه شد.")
            else:
                 print(f"WARNING: مرز ABF_CUTTING_LINES در مختصات {cl_center} در مرز ورق جلو یافت نشد یا قبلاً استفاده شده است. این موجودیت ممکن است به درستی پردازش نشود.")
...
def convert_coords_to_panel_system(entity_x, entity_y, panel_type, panel_xml_length, panel_xml_width, primary_bbox_panel, secondary_bbox_panel, sheet_border_front_bbox, sheet_border_back_bbox, tolerance=0.1):
    ...
    if in_front_sheet_bbox:
        reference_bbox_for_conversion = secondary_bbox_panel # Use panel's secondary bbox for front face
        determined_face = "5"
        # print("DEBUG:     Entity found in Front Sheet BBox. Using panel's secondary bbox as reference.")
    elif in_back_sheet_bbox:
        reference_bbox_for_conversion = primary_bbox_panel # Use panel's primary bbox for back face
        determined_face = "6"
        # print("DEBUG:     Entity found in Back Sheet BBox. Using panel's primary bbox as reference.")
    else:
    ...
    if reference_bbox_for_conversion is None:
         print("DEBUG:     Reference bbox for conversion is None. Conversion failed.")
    ...
    # Check if converted coordinates are within the expected XML panel bounds (with tolerance)
    check_tolerance = 1.0 # Increased tolerance for final bounds check
...
def create_pocket_xml(machines_element, entity, panel_length, panel_width, panel_type, primary_bbox_panel, secondary_bbox_panel, sheet_border_front_bbox, sheet_border_back_bbox, tolerance, config):
    ...
    # For Type=1, we MUST assign a face between 1-4, no exceptions.
    face = None

    # Use tolerance for determining which edge the pocket is on
    pocket_edge_tolerance = 20.0 # Tolerance for edge detection
    ...
    # If not near any edge with current tolerance, use the nearest edge
    if is_close_to_max_y_panel:
        face = "1" # Top Edge (Y=Width)
    elif is_close_to_min_y_panel:
        face = "2" # Bottom Edge (Y=0)
    elif is_close_to_max_x_panel:
        face = "3" # Right Edge (X=Length)
    elif is_close_to_min_x_panel:
        face = "4" # Left Edge
    else:
        # If not near any edge, find the nearest one
        dist_to_top = abs(panel_width - center_rel_y)
        dist_to_bottom = abs(center_rel_y)
        dist_to_right = abs(panel_length - center_rel_x)
        dist_to_left = abs(center_rel_x)
        
        # Find the minimum distance and corresponding face
        min_dist = min(dist_to_top, dist_to_bottom, dist_to_right, dist_to_left)
        if min_dist == dist_to_top:
            ...
            face = "3" # Right Edge
        else:  # min_dist == dist_to_left
            face = "4" # Left Edge
        print(f"DEBUG:   هشدار: Pocket در مختصات ({center_rel_x:.3f}, {center_rel_y:.3f}) به هیچ لبه نزدیک نبود. نزدیک ترین لبه انتخاب شد: Face {face}")

    # Snap the coordinates to the assigned edge
    xml_x_val = center_rel_x
    xml_y_val = center_rel_y

    # Snap to exact edge coordinate based on the assigned face
    if face == "2":  # Bottom Edge (Y=0)
        xml_y_val = 0.0
    elif face == "1":  # Top Edge (Y=Width)
        xml_y_val = panel_width
    elif face == "3":  # Right Edge (X=Length)
        xml_x_val = panel_length
    elif face == "4":  # Left Edge (X=0)
        xml_x_val = 0.0

    ET.SubElement(machines_element, "Machining",
                  Type="1", # Pocket/Routing
                  IsGenCode="2",
                  Face=face, # Will now always be 1-4
                  X=f"{xml_x_val:.3f}",
                  Y=f"{xml_y_val:.3f}",
                  Z=pocket_z_xml,
                  Diameter=f"{float(pocket_diameter_xml):.3f}",
                  Depth=f"{pocket_depth_xml:.3f}")
...
def create_groove_xml(machines_element, entity, panel_length, panel_width, panel_type, primary_bbox_panel, secondary_bbox_panel, sheet_border_front_bbox, sheet_border_back_bbox, tolerance, panel_thickness, config):
    ...
    groove_type = "4"
    is_gen_code = "2"
    # face = "5" # Assuming Face 5 for grooves based on sample - NOW DETERMINED BY SHEET BBOX
        ...
        if hasattr(entity, 'dxf'):
             try:
                 ...
                 # Add other entity types if needed (e.g., TEXT, INSERT)
                      ...
                      # print(f"DEBUG: Ignoring entity type: {entity.dxftype()} on layer {entity.dxf.layer}")
...
def dxf_to_custom_xml(input_file, config, panel_thickness=16.0): # Removed unused output_base_name
    ...
    try:
        ...
        for i, panel_group_info in enumerate(grouped_panels):
            ...
            output_file = f"{output_file_name_base}.xml"
...
class TestDXFToXMLConversion(unittest.TestCase):
    ...
    def test_convert_coords_to_panel_system(self):
        ...
        expected_x_xml_front = entity_y_dxf_front - panel_secondary_bbox_dxf[1] # 500 - 100 = 400
        expected_y_xml_front = entity_x_dxf_front - panel_secondary_bbox_dxf[0] # 1150 - 1100 = 50
...
class TerminalUI:
    ...
    def run(self):
        ...
        if choice is None or choice == 0:
            ...
            # Run the conversion
            dxf_to_custom_xml(selected_file, self.config, panel_thickness=panel_thickness_value)

            # After processing, prompt and exit
            input("\nPress Enter to exit...")
            sys.exit() # Exit the program after processing one file
    ...
    def _clear_screen(self):
        ...
        try:
            ...
            # Handle errors if the command itself fails or is not found
            # print(f"Warning: Could not clear screen using command. Error: {e}") # Optional: uncomment for debug
            # Fallback: Print many newlines if subprocess fails
            print("\n" * 100)
            # print("Warning: Using fallback clear screen method.") # Optional: uncomment for debug
        except Exception:
            # Fallback: Print many newlines for any other unexpected errors
            print("\n" * 100)
            # print("Warning: Could not clear screen due to an unexpected error. Using fallback.") # Optional: uncomment for debug



# --- Main Execution Block ---
if __name__ == '__main__':
    # Running for conversion (Terminal UI)
    # This block will now ALWAYS run the Terminal UI when the script is executed directly.
    # Unit tests can be run separately using 'python -m unittest your_script_name.py'
