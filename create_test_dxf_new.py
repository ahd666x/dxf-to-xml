...
def create_test_dxf():
    ...
    for pos in corner_holes:
        hole = msp.add_circle(pos, radius=5)  # D10 = radius 5
    ...
    for pos in shelf_holes_left + shelf_holes_right:
        hole = msp.add_circle(pos, radius=2.5)  # D5 = radius 2.5
    ...
    for center_x, center_y in hinge_positions:
        ...
        for dx, dy in [(-15,-15), (15,-15), (-15,15), (15,15)]:
            hole = msp.add_circle((center_x + dx, center_y + dy), radius=4)  # D8 = radius 4
            hole.dxf.layer = 'ABF_D8'
    
    # 4. Cable management holes with different sizes
    cable_holes = [
        # Large cable holes (D30)
        (700, 150, 15, 'ABF_D30'),  # D30 = radius 15
        (700, 850, 15, 'ABF_D30'),
        # Medium cable holes (D20)
        (700, 300, 10, 'ABF_D20'),  # D20 = radius 10
        (700, 700, 10, 'ABF_D20'),
        # Small cable holes (D10)
        (700, 450, 5, 'ABF_D10'),   # D10 = radius 5
        (700, 550, 5, 'ABF_D10')
    ]
    ...
    for i in range(-2, 3):
        for j in range(-2, 3):
            if abs(i) + abs(j) <= 3:  # Diamond shape
                ...
                hole = msp.add_circle((x, y), radius=3)  # D6 = radius 3
    ...
    for angle in range(0, 360, 45):  # 8 holes in circle
        ...
        hole = msp.add_circle((x, y), radius=4)  # D8 = radius 4
        hole.dxf.layer = 'ABF_D8'
...
if __name__ == '__main__':
    try:
        print("Starting test DXF creation...")
        
        doc = create_test_dxf()
        print("Drawing created in memory")
        ...
        print(f"Successfully saved DXF file to: {output_path}")
        
        # Extra validation - try to read the file back
        doc2 = ezdxf.readfile(output_path)
        validate_drawing(doc2)
        print("Final validation successful - file contains all required entities")
        
    except Exception as e:
        import traceback
        print(f"Error creating test DXF:\n{str(e)}")
